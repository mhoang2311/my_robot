...
class Run_Nav:
    def __init__(self):
        ...
        #self.mode_run = rospy.get_param('~mode_run','square')
        #self.mode_run = rospy.get_param('~mode_run','num8')
            
    def spin(self): #ok
        rospy.loginfo("[ROS] Start Navigation Test")
        self.update()
    ...
    def run_circle(self):
        pass
        rospy.loginfo("[ROS] Start Navigation Test: One Circle")
        ...
        rospy.loginfo('[ROS] timeOneCircle = ' + str(timeOneCircle))
        #
        move_cmd = Twist()
        ...
        while not rospy.is_shutdown():
            ...
            if time_duration > timeOneCircle:
                rospy.signal_shutdown('[ROS] Done node_nav_Circle') #PASSED
            #
            rospy.loginfo('[ROS] time duration = ' + str(time_duration))
            rate.sleep()   
    ...
    def run_square(self):
        ...
        time_start = rospy.Time.now()
        rospy.loginfo('[ROS] timeOneSquare = ' + str(timeOneSquare))
        #
        move_cmd = Twist()
        move_cmd.linear.x   = v     # v = 0.5 m/s
        move_cmd.angular.z  = w    # w = 0.5 rad/s
    ...
    def run_num8(self):
        ...
        timeOneNum8 = 28  #s
        time_start = rospy.Time.now()
        rospy.loginfo('[ROS] timeOneNum8 = ' + str(timeOneNum8))
        #
        move_cmd = Twist()
        move_cmd.linear.x   = v     # v = 0.5 m/s
        move_cmd.angular.z  = w    # w = 0.5 rad/s
